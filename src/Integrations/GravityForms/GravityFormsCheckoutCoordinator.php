<?php
/**
 * Gravity Forms hosted checkout coordinator.
 *
...
/**
 * Creates or safely resumes the Bachs checkout associated with a Gravity entry.
 */
final class GravityFormsCheckoutCoordinator {
	...
	public function __construct(
		...
		if ( 1 !== preg_match( '/\A[a-f0-9]{12}\z/D', $site_hash ) ) {
			throw new RuntimeException( 'Gravity Forms checkout site hash must contain 12 lowercase hexadecimal characters.' );
		}
	...
	/**
	 * Start or resume hosted checkout for a Gravity Forms entry.
	 *
	 * @param int    $entry_id    Gravity Forms entry identifier.
	 * @param int    $form_id     Gravity Forms form identifier.
	 * @param string $total       Trusted payment amount.
	 * @param string $currency    Trusted entry currency.
	 * @param string $success_url Browser success URL.
	 * @param string $cancel_url  Browser cancel URL.
	 * @return GravityFormsCheckoutResult
	 *
	...
	public function start(
		...
		string $success_url,
		string $cancel_url
	): GravityFormsCheckoutResult {
		if ( 1 > $entry_id || 1 > $form_id ) {
			throw new RuntimeException( 'Gravity Forms entry and form IDs must be positive.' );
		}
		...
		if ( null !== $latest ) {
			...
			if ( $existing instanceof GravityFormsCheckoutResult ) {
				return $existing;
			}
		...
		$intent  = $this->create_intent( $entry_id, $money, $attempt );
	 ...
	 *
	 * @param IntentRecord $latest      Latest persisted Gravity Forms intent.
	 * @param Money        $money       Current trusted entry amount.
	 * @param string       $success_url Browser success URL.
	 * @return GravityFormsCheckoutResult|IntentRecord|null
	 *
	...
	private function resume_or_classify_existing(
		...
		Money $money,
		string $success_url
	): GravityFormsCheckoutResult|IntentRecord|null {
		$intent = $latest->intent();

		if ( ApplicationStatus::APPLIED === $intent->application_status() ) {
			throw new RuntimeException( 'Gravity Forms payment intent is already applied.' );
		}
		...
		if (
			...
			throw new RuntimeException( 'Gravity Forms payment intent requires review before another checkout can be created.' );
		}
		...
		if ( 'open' === $status ) {
			if ( ! $same_snapshot ) {
				throw new RuntimeException( 'Gravity Forms entry amount changed while an earlier Bachs checkout is still open.' );
			}
		...
		if ( 'completed' === $status ) {
			return new GravityFormsCheckoutResult( $success_url, $intent->uuid(), $session->checkout_id() );
		}
	...
	private function create_provider_checkout(
		...
		if ( ! $this->intents->attach_checkout( $row_id, $session->checkout_id() ) ) {
			throw new RuntimeException( 'Bachs checkout was created but could not be attached to the Gravity Forms payment intent.' );
		}

		return new GravityFormsCheckoutResult( $url, $intent->uuid(), $session->checkout_id() );
	}

	/**
	 * Create a new immutable Gravity Forms payment intent.
	 *
	 * @param int   $entry_id Gravity Forms entry identifier.
	 * @param Money $money    Trusted entry money.
	 * @param int   $attempt  Logical checkout attempt.
	 * @return PaymentIntent
	 */
	private function create_intent( int $entry_id, Money $money, int $attempt ): PaymentIntent {
		$uuid            = ( $this->uuid_generator )();
		$reference       = 'etp_bch_' . ( $this->reference_generator )();
		$idempotency_key = sprintf(
			'etp:%s:gf:%d:checkout:%d',
			$this->site_hash,
			$entry_id,
			$attempt
		);
	...
	private static function result_from_session(
		...
		return new GravityFormsCheckoutResult( $url, $intent->intent()->uuid(), $session->checkout_id() );
	}
}
