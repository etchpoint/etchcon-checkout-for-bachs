<?php
/**
 * Gravity Forms checkout coordinator tests.
 *
...
use Etchpoint\BachsIntegrations\Integrations\GravityForms\GravityFormsCheckoutCoordinator;
...
/**
 * Verifies idempotent Gravity Forms checkout creation and retry behavior.
 */
final class GravityFormsCheckoutCoordinatorTest extends TestCase {
	/**
	 * New entries persist an intent before creating provider checkout.
	 *
	 * @return void
	 */
	public function test_new_entry_creates_intent_and_attaches_checkout(): void {
		...
		$checkouts->expects( self::once() )
			...
			->willReturn( self::checkout_session( 'chk_gf_1', 'https://checkout.example/gf', 'open' ) );
		$intents->expects( self::once() )
		...
		self::assertSame( 'gravity_forms', $captured->integration() );
		self::assertSame( 'etp:abcdef123456:gf:91:checkout:1', $captured->idempotency_key() );
		self::assertSame( 'https://checkout.example/gf', $result->redirect_url() );
	}
	 ...
	 * @return GravityFormsCheckoutCoordinator
	 */
	private static function coordinator(
		CheckoutIntentStore $intents,
		CheckoutProvider $checkouts
	): GravityFormsCheckoutCoordinator {
		return new GravityFormsCheckoutCoordinator(
			$intents,
			$checkouts,
			Environment::SANDBOX,
			'abcdef123456',
	...
	/**
	 * Build a persisted Gravity Forms intent record.
	 *
	...
	private static function intent_record(
		...
		$intent = PaymentIntent::create(
			...
			'etp:abcdef123456:gf:91:checkout:' . $attempt,
