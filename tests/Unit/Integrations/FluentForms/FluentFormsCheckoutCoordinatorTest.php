<?php
/**
 * Fluent Forms checkout coordinator tests.
 *
...
use Etchpoint\BachsIntegrations\Integrations\FluentForms\FluentFormsCheckoutCoordinator;
...
/**
 * Verifies idempotent Fluent Forms checkout creation and retry behavior.
 */
final class FluentFormsCheckoutCoordinatorTest extends TestCase {
	/**
	 * New submissions persist an intent before creating provider checkout.
	 *
	 * @return void
	 */
	public function test_new_submission_creates_intent_and_attaches_checkout(): void {
		...
		$intents->expects( self::once() )
			...
			->with( 'fluent_forms', 'submission', '91' )
			->willReturn( null );
		$intents->expects( self::once() )
			->method( 'create' )
			->willReturnCallback(
		...
		$checkouts->expects( self::once() )
			...
			->willReturn( self::checkout_session( 'chk_ff_1', 'https://checkout.example/ff', 'open' ) );
		$intents->expects( self::once() )
		...
		self::assertSame( 'fluent_forms', $captured->integration() );
		self::assertSame( 'etp:abcdef123456:ff:91:checkout:1', $captured->idempotency_key() );
		self::assertSame( 'https://checkout.example/ff', $result->redirect_url() );
	}
	 ...
	 * @return FluentFormsCheckoutCoordinator
	 */
	private static function coordinator(
		CheckoutIntentStore $intents,
		CheckoutProvider $checkouts
	): FluentFormsCheckoutCoordinator {
		return new FluentFormsCheckoutCoordinator(
			$intents,
			$checkouts,
			Environment::SANDBOX,
			'abcdef123456',
	...
	/**
	 * Build a persisted Fluent Forms intent record.
	 *
	...
	private static function intent_record(
		...
		$intent = PaymentIntent::create(
			...
			'etp:abcdef123456:ff:91:checkout:' . $attempt,
